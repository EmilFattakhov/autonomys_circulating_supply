...
const ALLOCATIONS = {
  ...
  testnets: {
    ...
    stakeWars2: { percent: 0.30, tokens: 3_000_000, vesting: 'immediate' }
  },
...
// Calculate farming rewards based on block height using the actual Subspace formula
// Formula: reference_subsidy = initial_subsidy * e^(-initial_subsidy * (n - decay_block_start) / max_issuance_tokens)
function calculateFarmingRewards(currentDate = new Date()) {
  // Mainnet farming started November 6, 2024 12:30PM EST
  const FARMING_START_DATE = new Date('2024-11-06T17:30:00Z'); // 12:30PM EST = 17:30 UTC
  
  // If before farming start, no rewards
  if (currentDate < FARMING_START_DATE) {
    return 0;
  }
  
  const millisecondsSinceFarmingStart = Math.max(0, currentDate - FARMING_START_DATE);
  const totalBlocks = Math.floor(millisecondsSinceFarmingStart / (BLOCK_TIME_SECONDS * 1000));
  
  if (totalBlocks === 0) return 0;
  
  // Mainnet parameters (estimated based on 40-year issuance and 350M total rewards)
  // These should be updated with actual mainnet parameters when available
  const INITIAL_SUBSIDY = 5; // ATC per block (estimated)
  const DECAY_BLOCK_START = 0; // Start decay from block 0 on mainnet
  const MAX_ISSUANCE_TOKENS = 350_000_000; // Total farmer rewards allocation
  
  let totalRewards = 0;
  
  // Calculate rewards for each block using the exponential decay formula
  for (let blockNumber = 0; blockNumber < totalBlocks; blockNumber++) {
    const referenceSubsidy = INITIAL_SUBSIDY * Math.exp(-INITIAL_SUBSIDY * (blockNumber - DECAY_BLOCK_START) / MAX_ISSUANCE_TOKENS);
    
    // Each block typically has ~10 recipients (1 proposer + 9 voters)
    // Proposer gets 10% of reference subsidy, each voter gets 10%
    const blockReward = referenceSubsidy;
...
function calculateVestedAmount(allocation, vestingType, currentDate = new Date(), tgeDate = new Date()) {
...
export function calculateCirculatingSupply(currentDate = new Date(), tgeDate = TGE_DATE) {
  ...
  circulatingSupply += ALLOCATIONS.testnets.stakeWars2.tokens;
